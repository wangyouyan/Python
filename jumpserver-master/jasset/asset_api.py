...
from jasset.models import ASSET_STATUS, ASSET_TYPE, ASSET_ENV, IDC, AssetRecord,Idrac
...
def db_asset_alert(asset, username, alert_dic):
    ...
    for field, value in alert_dic.iteritems():
        print field
        if  field not in ["Idrac_user","Idrac_ip","Idrac_passwd"]:
        ...
        if field in ["Idrac_user","Idrac_ip","Idrac_passwd"]:
            field_name = Idrac._meta.get_field_by_name(field)[0].verbose_name
            print value
            alert_info= [ field_name, value[0], value[1] ]
        ...
        elif field in ['username', 'password','ID','id']:
            continue
